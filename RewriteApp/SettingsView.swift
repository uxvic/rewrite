    ...
    private var generalSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Toggle("Smart send (rewrite vs. fulfill a request)", isOn: $settings.smartIntent)
                .font(.system(size: 13)).foregroundStyle(Theme.textPrimary)
            desc("With Smart on, a plain send in Rewrite figures out whether you gave it text to polish or a request to carry out (e.g. “draft an email…”) and does the right thing. Explicit styles always rewrite literally.")
            desc("Clipboard history starts automatically and stays only on this Mac. It records normal text copies, not concealed or transient pasteboard items. Turn it off here to stop future capture.")
