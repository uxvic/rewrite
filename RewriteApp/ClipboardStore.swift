...
/// bounded history of ordinary text copies. This is intentionally separate from
/// the rewrite history: clipboard items never reach a model provider unless the
/// user explicitly selects one and sends it through the Rewrite composer.
final class ClipboardStore: ObservableObject {
    ...
    /// count is only a baseline, so an item copied before the app started is not
    /// silently retroactively added to history.
    func start() {
    ...
    private func pollPasteboard() {
        ...
        // capture something copied while the user had history turned off.
        ...
        // Re-copying an older item should move it to the top, not create noisy
        // duplicate rows. The exact text is preserved, including line breaks.
